...
def solve_smt(instance, timeout):
    start = time.time()
    result, max_dist, paths,costs = solve(instance)
    end = time.time()
    if result:
        print(f"Instance {instance} solved in {round(end-start, 2)}s with a max_dist of {max_dist}")
        for ix, path in enumerate(paths):
            print(f"\t path {ix}: {path} - cost: {costs[ix]}")
    else:
        print(f"instance {instance} not solved (took {round(end-start, 2)}s)")

    result_dict = {
        "time" : int(min(round(end-start, 2), 300)),
        "optimal" : result,
        "obj" : max_dist,
        "sol" : paths 
    }
