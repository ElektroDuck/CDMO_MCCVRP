...
def sat_model(num_couriers, num_items, capacity, size, distances_int, upper_bound, lower_bound, display_solution=True, timeout_duration=300, search="Base"):
    ...
    if search == "Base":
        ...
        while solver.check() == z3.sat:
            ...
            new_upper_bound = obj_value - 1
            new_upper_bound_bin = int_to_bin(new_upper_bound, num_bits(new_upper_bound))

            solver.pop()
            solver.push()
            solver.add(AllLessEq_bin(distances, new_upper_bound_bin))

            Ord = evaluate(model, order)
            Ass = evaluate(model, assignments)
            deliveries = retrieve_routes(Ord, Ass)

            res_dict = creat_solution_dict(deliveries, 300, False, obj_value)
        ...
        new_lower_bound_bin = int_to_bin(new_lower_bound, num_bits(new_lower_bound))
        new_upper_bound_bin = int_to_bin(new_upper_bound, num_bits(new_upper_bound))

        while new_lower_bound <= new_upper_bound:
            mid = (new_lower_bound + new_upper_bound)//2
            ...
            if now >= timeout:
                break
            solver.set('timeout', millisecs_left(now, timeout))
            print(f"Trying with bounds: [{new_lower_bound}, {new_upper_bound}] and posing obj_val <= {mid}")
            ...
            if solver.check() == z3.sat:
                ...
                new_lower_bound = mid + 1
                new_lower_bound_bin = int_to_bin(new_lower_bound, num_bits(new_lower_bound))

            solver.pop()
            solver.push()
            solver.add(AllLessEq_bin(distances, new_upper_bound_bin))
            solver.add(AtLeastOneGreaterEq_bin(distances, new_lower_bound_bin))
...
if __name__ == "__main__":
    ...
    ub = int(args.ub)
    lb = int(args.lb)
    search = args.search # Binary / Base

    num_vehicles, num_clients, vehicles_capacity, packages_size, distances = extract_data_from_dat(instance_number)
    obj_value, solving_time, deliveries = sat_model(num_vehicles, num_clients, vehicles_capacity, packages_size, distances, ub, lb, search=search)
