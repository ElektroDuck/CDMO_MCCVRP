...
def get_mip_model_path(model_name):
    mip_model_path = os.path.join(BASE_PATH, "MIP", model_name)
...
def mip_extract_route(succ_matrix, prev_idx, prev=[]):
    ...
    return mip_extract_route(succ_matrix, next_idx, prev)

def reconstruct_mip_minizinc_solution(succ_matrix, num_vehicles, num_clients, distance_matrix):
    ...
    solution = {}
    for i in range(0, num_vehicles):
        solution[i] = mip_extract_route(succ_matrix[:,:,i], len(succ_matrix)-1)
...
def get_gurobi_env():
    ...
    params = {
    "WLSACCESSID": 'INSERT HERE',
    "WLSSECRET": 'INSERT HERE',
    "LICENSEID": 'INSERT HERE',
    }
...
def solve_cp(model_name, solver_id, instance_data, timeout_time, use_sb): 
    ...
    if check_simmetry(matrix_dist) and use_sb:
...
def solve_mip_minizinc(model_name, solver_id, instance_data, timeout_time):
    ...
    solution = reconstruct_mip_minizinc_solution(succ_matrix, num_vehicles, num_clients, matrix_dist)
...
def solve_mip(instance_data, timeout_time, model, solver="gecode"):
    if model == "gurobi":
        ...
        return solve_mip_minizinc("MIP_model.mzn", solver, instance_data, timeout_time)
    else:
...
def solve_sat(instance_data, instance_n, timeout_time, model_name):
    distances, num_vehicles, num_clients, vehicles_capacity, packages_size = instance_data["distances"], instance_data["num_vehicles"], instance_data["num_clients"], instance_data["vehicles_capacity"], instance_data["packages_size"]
    low_bound, up_bound, _ = calculate_mccrp_bounds(distances)
    flag = False
    try:
        # Calling the SMT solver as a subprocess, so we can apply timeout
        subprocess.call(["python3", "./SAT/sat_model.py", str(instance_n), str(up_bound), str(low_bound), model_name], timeout=timeout_time)
    except subprocess.TimeoutExpired:
        print("Solution TIMEOUT")
        flag = True
